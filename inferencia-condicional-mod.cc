...
bool seleccionarVariablesCondicionales(int& maskC, int& valC, int N) {
  ...
  for (int i = 0; i < variables_condicionales; ++i) {
    ...
    if (variable > N || variable < 0 || (valor != 0 && valor != 1)) {
      std::cerr << "Error al seleccionar variable condicionada, debe estar entre 1-" << N << " y con valor 0 o 1.\n";
      return false;
    }
    --variable;
...
bool seleccionarVariablesInteres(int& maskI, int& valI, int N) {
  ...
  for (int i = 0; i < variables_interes; ++i) {
    ...
    std::cin >> variable >> valor;
    if (variable > N || variable <= 0  || (valor != 0 && valor != 1)) {
      std::cerr << "Error al seleccionar variable de interés, debe estar entre 1-" << N << " y con valor 0 o 1.\n";
      return false;
    }
    --variable;
...
int main() {
  ...
  if (!(seleccionarVariablesInteres(maskI, valI, N))) return 1;
